...
export async function createConversation(req: Request, res: Response): Promise<void> {
  ...
  await withAIStream(
    ...
    201,
    async (emitChunk) => {
      const scenario = await roleplayService.generateScenario(topic, level, emitChunk);
...
export async function sendMessage(req: Request, res: Response): Promise<void> {
  ...
  try {
    await withAIStream(
      ...
      200,
      async (emitChunk) => {
        const aiResponse = await roleplayService.chat(getUserId(req), id, message.trim(), emitChunk);
...
export async function endConversation(req: Request, res: Response): Promise<void> {
  ...
  await withAIStream(
    ...
    200,
    async (emitChunk) => {
      const result = await roleplayService.summarizeConversation(getUserId(req), id, emitChunk);
      await updateStreak(getUserId(req));
      const xp = calculateXP('PRONUNCIATION');
      ...
      return { conversation: result.conversation, analysis: result.analysis, xpEarned: xp };
    },
    (result) => result,
  );
}
